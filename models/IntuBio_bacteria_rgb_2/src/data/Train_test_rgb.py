...
for f in val_folders: #loop igennem forskellige test eg. staph LOQ4
    tests=sorted(os.listdir(os.path.join(Val,f))) #list alle brønde i annotated
    tests_rgb=sorted(os.listdir(os.path.join(IntuBio_bacteria_rgb,f))) #list alle brønde i Rgb
    test_folder=zip(tests,tests_rgb)
    for sf,sf_rgb in test_folder:
        well=os.path.join(Val,f,sf) #path til en brønd eg: /work3/s174182/train_data/Annotated_segmentation_patch_balanced/train/BC_accuracy_linearity_20220607-14230983/A1_D4_/
        ...
        #Create a folder where to move the new RGB images
        rgb_imgdest=os.path.join(IntuBio_val_folder,f,sf,"img")
        os.makedirs(rgb_imgdest,exist_ok=True)
        rgb_maskdest=os.path.join(IntuBio_val_folder,f,sf,"mask")
        os.makedirs(rgb_maskdest,exist_ok=True)
